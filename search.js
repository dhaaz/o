...
const searchitem = async (srchquery) => {
    const res = await fetch(`https://api.themoviedb.org/3/search/multi?api_key=${myApi}&language=en-US&query=${srchquery}&page=1`);
...
const searchMoviefun = (movie) => {
    ...
    <a class="posterlink" href="${url}"> <img class="poster" data-id="${movie.id
         ...
         <div class="date_rating">
             ...
             <div class="category category_search">Film</div>
             </div>
         </div>`;
};
...
const searchTVfun = (movie) => {
    ...
    <a class="posterlink" href="${url}"> <img class="poster" data-id="${movie.id
         ...
         <div class="date_rating tvshows_date_rating">
             ...
             <div class="category category_search recommendTvShow_category">Series</div>
             </div>
         </div>`;
};
    ...
    if (searchBtn.value != "") {
        ...
        searchitem(searchQuery).then((movies) => {
            movies.forEach((moviee) => {
                ...
                if (moviee.media_type == "movie" && 'release_date' in moviee && moviee.poster_path != null) {
