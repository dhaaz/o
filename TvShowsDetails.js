...
const html2 = function (tvshow) {
    ...
    let url = `./SeasonEpisode.html?/Z${+ encodeURIComponent(tvshow.number_of_seasons)}-id=${+ encodeURIComponent(tvshow.id)}`
    document.title = `${tvshow.name + " " + "(" + (dateFormatter(
        tvshow.first_air_date)) + ")" + " " + "|" + " " + "Cinemaa"}`
    ...
    <img class="movie_details_poster" src="https://image.tmdb.org/t/p/w500/${tvshow.poster_path}" alt="title">
    <div class="movie_details_about">
        ...
        <div class="date_rating tvshowdetail_date_rating">
            <p class="time">${tvshow.number_of_seasons} Season</p><span class="dot dot2 dot3"></span>
            <p class="time">${tvshow.episode_run_time[0]} menit per episode</p><span class="dot dot2 dot3"></span>
            <p class="time">${tvshow.status}</p><span class="dot dot2 dot3"></span>
            <p class="date">${dateFormatter(tvshow.first_air_date)} - ${dateFormatter(tvshow.last_air_date)}</p><span class="dot dot2 dot3"></span>
            <p class="rating">${tvshow.vote_average}<span><svg xmlns="http://www.w3.org/2000/svg" width="10"
    ...
    <a class="posterlink" href="${url}"> <img class="poster" data-id="${mov.id
         ...
         <div class="date_rating">
            ...
            mov.first_air_date
             ...
             <p class="rating recommendTvShow_date">${averagVoteformat(mov.vote_average)}<span><svg xmlns="http://www.w3.org/2000/svg" width="10"
                         height="10" fill="Yellow" class="star bi-star-fill" viewBox="0 0 16 16">
                         <path
                             d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
                     </svg></span></p>
             <div class="category recommendTvShow_category">Series</div>
             </div>
         </div>`;
};
    ...
    <a class="posterlink" href="${url}"> <img class="poster" data-id="${movie.id
         ...
         <div class="date_rating">
            ...
            movie.first_air_date
             ...
             <p class="rating recommendTvShow_date">${averagVoteformat(movie.vote_average)}<span><svg xmlns="http://www.w3.org/2000/svg" width="10"
                         height="10" fill="Yellow" class="star bi-star-fill" viewBox="0 0 16 16">
                         <path
                             d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
                     </svg></span></p>
